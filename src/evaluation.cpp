...
Bitboard static_pawn_attacks(const Bitboard pawns) {
    Bitboard attacks =
      pawns.shift_relative(color, Direction::NorthEast) | pawns.shift_relative(color, Direction::NorthWest);
    return attacks;
}
...
PScore evaluate_pawns(const Position& pos) {
    ...
    for (Square sq : pawns) {
        ...
        if (stoppers.empty()) {
            ...
            if (pos.attack_table(color).read(push).popcount() > pos.attack_table(them).read(push).popcount()) {
...
PScore evaluate_pawn_push_threats(const Position& pos) {
    ...
    Bitboard push_attacks =
      ...
      pushable.shift_relative(color, Direction::North).shift_relative(color, Direction::NorthWest);

    eval += PAWN_PUSH_THREAT_KNIGHT * (push_attacks & pos.bitboard_for(opp, PieceType::Knight)).ipopcount();
    eval += PAWN_PUSH_THREAT_BISHOP * (push_attacks & pos.bitboard_for(opp, PieceType::Bishop)).ipopcount();
    eval += PAWN_PUSH_THREAT_ROOK * (push_attacks & pos.bitboard_for(opp, PieceType::Rook)).ipopcount();
    eval += PAWN_PUSH_THREAT_QUEEN * (push_attacks & pos.bitboard_for(opp, PieceType::Queen)).ipopcount();
...
PScore evaluate_pieces(const Position& pos) {
    ...
    constexpr Bitboard early_ranks     = color == Color::White ? Bitboard::rank_mask(1) | Bitboard::rank_mask(2)
    ...
    for (PieceId id : pos.get_piece_mask(color, PieceType::Bishop)) {
        ...
        eval += BISHOP_PAWNS[std::min(static_cast<usize>(8),
...
PScore evaluate_outposts(const Position& pos) {
    ...
    constexpr Bitboard viable_outposts_ranks =
      color == Color::White ? Bitboard::rank_mask(3) | Bitboard::rank_mask(4) | Bitboard::rank_mask(5)
    ...
    PScore eval = PSCORE_ZERO;
    eval +=
      OUTPOST_KNIGHT_VAL * static_cast<i32>((pos.bitboard_for(color, PieceType::Knight) & viable_outposts).popcount());
    eval +=
      OUTPOST_BISHOP_VAL * static_cast<i32>((pos.bitboard_for(color, PieceType::Bishop) & viable_outposts).popcount());
    return eval;
}
...
PScore evaluate_threats(const Position& pos) {
    ...
    eval += KNIGHT_THREAT_BISHOP * (pos.bitboard_for(opp, PieceType::Bishop) & knight_attacks).ipopcount();
    eval += KNIGHT_THREAT_ROOK * (pos.bitboard_for(opp, PieceType::Rook) & knight_attacks).ipopcount();
    eval += KNIGHT_THREAT_QUEEN * (pos.bitboard_for(opp, PieceType::Queen) & knight_attacks).ipopcount();

    Bitboard bishop_attacks = pos.attacked_by(color, PieceType::Bishop);
    eval += BISHOP_THREAT_KNIGHT * (pos.bitboard_for(opp, PieceType::Knight) & bishop_attacks).ipopcount();
    eval += BISHOP_THREAT_ROOK * (pos.bitboard_for(opp, PieceType::Rook) & bishop_attacks).ipopcount();
    eval += BISHOP_THREAT_QUEEN * (pos.bitboard_for(opp, PieceType::Queen) & bishop_attacks).ipopcount();
...
Score evaluate_white_pov(const Position& pos, const PsqtState& psqt_state) {
    ...
    usize       phase =
      pos.piece_count(Color::White, PieceType::Knight) + pos.piece_count(Color::Black, PieceType::Knight) +
      pos.piece_count(Color::White, PieceType::Bishop) + pos.piece_count(Color::Black, PieceType::Bishop) +
      2 * (pos.piece_count(Color::White, PieceType::Rook) + pos.piece_count(Color::Black, PieceType::Rook)) +
      4 * (pos.piece_count(Color::White, PieceType::Queen) + pos.piece_count(Color::Black, PieceType::Queen));
    ...
    eval += evaluate_pawn_push_threats<Color::White>(pos) - evaluate_pawn_push_threats<Color::Black>(pos);
    eval += evaluate_potential_checkers<Color::White>(pos) - evaluate_potential_checkers<Color::Black>(pos);
