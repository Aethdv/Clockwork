...
struct alignas(16) PieceList {
    ...
    [[nodiscard]] u8x16 to_vector() const { return std::bit_cast<u8x16>(array); }

    [[nodiscard]] PieceMask mask_valid() const { return PieceMask{to_vector().nonzeros().to_bits()}; }
...
struct RookInfo {
    ...
    [[nodiscard]] constexpr bool is_clear() const { return !aside.is_valid() && !hside.is_valid(); }
...
public:
    ...
    [[nodiscard]] const Byteboard& board() const { return m_board; }
    [[nodiscard]] const Wordboard& attack_table(Color color) const { return m_attack_table[static_cast<usize>(color)]; }
    ...
    [[nodiscard]] Color    active_color() const { return m_active_color; }
    [[nodiscard]] Square   en_passant() const { return m_enpassant; }
    [[nodiscard]] RookInfo rook_info(Color color) const { return m_rook_info[static_cast<usize>(color)]; }
    [[nodiscard]] HashKey  get_hash_key() const { return m_hash_key; }
    [[nodiscard]] HashKey  get_pawn_key() const { return m_pawn_key; }
    [[nodiscard]] HashKey  get_non_pawn_key(Color color) const { return m_non_pawn_key[static_cast<usize>(color)]; }
    [[nodiscard]] HashKey  get_major_key() const { return m_major_key; }
    [[nodiscard]] HashKey  get_minor_key() const { return m_minor_key; }

    [[nodiscard]] Square king_sq(Color color) const { return piece_list_sq(color)[PieceId{0}]; }

    [[nodiscard]] size_t king_side(Color color) const { return king_sq(color).file() >= 4; }

    [[nodiscard]] PieceType piece_at(Square sq) const { return m_board[sq].ptype(); }
    ...
    [[nodiscard]] bool is_in_check() const { return !checker_mask().empty(); }
    ...
    [[nodiscard]] PieceMask get_piece_mask(Color color) const { return piece_list(color).mask_valid(); }
    ...
    [[nodiscard]] Wordboard create_attack_table_superpiece_mask(Square sq, CreateSuperpieceMaskInfo cmi) const;

    [[nodiscard]] PieceType pt_of(Color color, PieceId id) const { return piece_list(color)[id]; }

    [[nodiscard]] usize piece_count(Color color) const { return 16 - piece_count(color, PieceType::None); }
    ...
    [[nodiscard]] Position move(Move m) const { return move<false>(m, nullptr); }
    [[nodiscard]] Position move(Move m, PsqtState& psqt_state) const { return move<true>(m, &psqt_state); }
    ...
    static std::optional<Position> parse(std::string_view board, std::string_view color, std::string_view castle,
...
private:
    ...
    void incrementally_mutate_piece(bool old_color, PieceId old_id, Square sq, bool new_color, Place p,
                                    PsqtUpdates& updates);
    void incrementally_move_piece(bool color, Square from, Square to, Place p, PsqtUpdates& updates);
